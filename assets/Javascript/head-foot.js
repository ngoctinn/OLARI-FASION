...
function search() {
  ...
  window.location =
    search === "" ? "./san-pham.html" : `./san-pham.html?search=${search}`;
}
...
loginPW.addEventListener("click", (e) => {
  // Corrected ID
  e.preventDefault();
  formContainer.classList.remove("pw");
});
...
const checkValidation = (value) => {
  ...
  switch (value) {
    case 1:
      valid &=
        kiemTraRong("userNameLG", "error_userNameLG_empty") &
        kiemTraRong("passwordLG", "error_passwordLG");
      valid &= kiemTraDangNhap(
        "userNameLG",
        "error_userNameLG",
        "passwordLG",
        "error_passwordLG"
    ...
    case 2:
      //Kiểm tra rổng
      valid &=
        kiemTraRong("userNameSU", "error_userNameSU_empty") &
        kiemTraEmail("#emailSU", "#error_emailSU") &
        kiemTraRong("passwordSU", "error_passwordSU") &
        kiemTraRong("confirmPasswordSU", "error_confirmPasswordSU_empty");
      ...
      valid &= kiemTraKhopPass(
        "#passwordSU",
        "#confirmPasswordSU",
        "#error_confirmPasswordSU"
      );
...
const kiemTraUserName = (selectorValue, selectorError) => {
  ...
  if (inputText.value.length < 6 || inputText.value.length > 20) {
    document.querySelector(selectorError).innerHTML +=
      inputText.name + " từ 6 đến 20 kí tự! <br>";
    document.querySelector(selectorError).style.display = "block";
    khongHopLe = true;
  }
  if (!regexKiTu.test(inputText.value)) {
    document.querySelector(selectorError).innerHTML +=
      inputText.name + " có chứa các ký tự không hợp lệ!";
    document.querySelector(selectorError).style.display = "block";
    khongHopLe = true;
  }
  if (inputText.value.length >= 6 && usernameTonTai(inputText.value) !== -1) {
    document.querySelector(selectorError).innerHTML +=
      "Tên đăng nhập" + " đã tồn tại";
    document.querySelector(selectorError).style.display = "block";
    khongHopLe = true;
  }
  if (!khongHopLe) {
...
const kiemTraDangNhap = (
  ...
  if (index === -1) {
    document.getElementById(loginError).innerHTML =
      "Tên đăng nhập không tồn tại!!!";
    document.getElementById(loginError).style.display = "block";
    return false;
  }
  if (checkKhoa(document.getElementById(loginInput).value)) {
    document.getElementById(loginError).innerHTML =
      "Tên đăng nhập đang bị khóa. Vui lòng thử lại sau";
    document.getElementById(loginError).style.display = "block";
    return false;
  }
  ...
  switch (value) {
    ...
    case 3:
      suaMatKhau(
        ...
        document.getElementById("passwordFG").value
      );
      location.reload();
      break;
  }
