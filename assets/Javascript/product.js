...
function renderPage() {
  ...
  for (let i = start; i < end; i++) {
    contentDetail += `
            <div id="chi-tiet" class="chi-tiet">
                <div class="hinh-san-pham" onclick="showInfo(${
                  productsList[i].id
                })">
      ...
      productsList[i].id
                ...
                <div class="ten-san-pham pt-3" onclick="showInfo(${
                  productsList[i].id
                })">
                    ${productsList[i].name}
                </div>
  ...
  for (let i = start, j = 0; i < end; i++) {
    if (productsList[i].remain > 0)
      document.querySelectorAll(".mua-hang")[
        i % page.numOfProducts
      ].innerHTML = `
                <div class="input-group mb-3 d-flex justify-content-center pt-3">
                    <input type="number" name="" value="1" min="1" max="${Number(
                      productsList[i].remain
                    )}" class="amount amount-page text-center" onkeyup="checkKey(this, event);">
      ...
      document.querySelectorAll(".mua-hang")[
        i % page.numOfProducts
      ].innerHTML = `<div class="mb-3 pt-3">Hết hàng</div>`;
  }
  renderNumber();
}
...
function renderNumber() {
  ...
  let bottom =
    '<button class="btn btn-outline-danger" onclick="setPage(1)" id="first-page">&lt;</button>';
  ...
  (page.current === 1 && $("#first-page").css("visibility", "hidden")) ||
    $("#first-page").css("visibility", "visible");
  (page.current === soTrang && $("#last-page").css("visibility", "hidden")) ||
    $("#last-page").css("visibility", "visible");
...
function showInfo(id) {
                ...
                <div class="container">
                    <div class="row p-5">
                        <div class="col-5">
                            <img src=${
                              product.img
                            } class="d-block w-100" style="aspect-ratio: 1/1">
                        </div>
                        <div class="col-7">
                            <fieldset class="tieu-de mb-5">
                                <h2 class="mb-4">${product.name}</h2>
                                <h3 style="color: #FF4F81;">${getGia(
                                  product.price
                                )}</h3>
  ...
  if (product.remain > 0)
    document.getElementById("mua-hang-info").innerHTML = `
...
function changeNumber(input) {
  ...
  if (
    input === "+" &&
    amount.value + 1 <= document.getElementById("amount-info").max
  )
    amount.value++;
...
function addToCart(product, index = undefined) {
  ...
  if (index === undefined) amount = document.getElementById("amount-info");
  else amount = document.getElementsByClassName("amount-page")[index];

  if (amount.value === "") dangerMessage("Vui lòng nhập số lượng cần thêm");
  else if (amount.value == 0)
    dangerMessage("Vui lòng thêm nhiều hơn 1 sản phẩm");
  else {
